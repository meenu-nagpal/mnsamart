...
//     singlesubcat=[...new Set(SubCategory)]
//     console.log( "singlesubcat"+singlesubcat)

// singlesubsubcat=[...new Set(SubSubCategory)]
// console.log("singlesubsubcat"+singlesubsubcat)
    ...
    var uniquecat = unique.map((k) => {
        ...
        return (<li><button value={k} onClick={(e) => { Handler(e.target.value);setsinglecat(e.target.value) }}>{k}</button></li>)
    })
    ...
    function showdata(e) {
        ...
        // e.target.name == "productimages" && (productimages= e.target.files);
...
alert("product uploaded")
        ...
        for(var f of productimages)
        {
            formData.append("productimages",f);
        }

        axios.post("http://localhost:9000/uploadfiles",formData, {
            headers: {
